...
ylabel('f(n)');
title('采样周期为1');
X1 = f1*exp(-1j*tn1'*w); %傅里叶变换，t'的每个相邻时间差是0.0001，足够小，我们近似认为此时级数等于积分
subplot(4,2,4);
plot(w,X1);
xlabel('Ω');
ylabel('X1(jΩ)');
...
ylabel('f(n)');
title('采样周期为pi/2');
X2 = f2*exp(-1j*tn2'*w); %傅里叶变换，t'的每个相邻时间差是0.0001，足够小，我们近似认为此时级数等于积分
subplot(4,2,6);
plot(w,X2);
xlabel('Ω');
ylabel('X2(jΩ)');
...
ylabel('f(n)');
title('采样周期为2');
X3 = f3*exp(-1j*tn3'*w); %傅里叶变换，t'的每个相邻时间差是0.0001，足够小，我们近似认为此时级数等于积分
subplot(4,2,8);
plot(w,X3);
xlabel('Ω');
ylabel('X3(jΩ)');
